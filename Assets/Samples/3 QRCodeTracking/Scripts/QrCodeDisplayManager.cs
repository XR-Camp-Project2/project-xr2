...
public class QrCodeDisplayManager : MonoBehaviour
{
    ...
    private bool _isSpawningCharacter = false;
    ...
    private async void UpdateMarkers()
    {
        ...
        foreach (var qrResult in qrResults)
        {
            ...
            if(_spawnedCharacter == null && !_isSpawningCharacter)
            {
                _isSpawningCharacter = true;
        ...
        // 如果沒有檢測到QR，且動畫未完成，則銷毀角色
        if(qrResults.Length == 0 && _spawnedCharacter != null && !_animationCompleted)
        {
